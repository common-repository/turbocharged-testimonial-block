		...
		save( props ) {
			...
			const rowClasses = classnames(
				'flex',
				{
					...
					[ `${ borderRadius }` ]: ! style.includes( 'circle' ) && ! customBorderRadius,
					[ `${ borderWidth }` ]: ! style.includes( 'circle' ) && ! customBorderWidth,
			...
			const contentColumnClasses = classnames(
				'p-10 relative',
				{
					...
					[ `${ borderRadius }` ]: style === 'circle' && ! customBorderRadius || style === 'circle-left' && ! customBorderRadius || style === 'circle-right' && ! customBorderRadius,
			...
			const contentColumnStyle = {
				...
				borderRadius: !! customBorderRadius && style.includes( 'circle' ) ? customBorderRadius : null,
				borderWidth: !! customBorderWidth && style.includes( 'circle' ) ? customBorderWidth : null,
				boxShadow: useCustomBoxShadow && style.includes( 'circle' ) ? `${ customBoxShadow.x }px ${ customBoxShadow.y }px ${ customBoxShadow.blur }px ${ customBoxShadow.spread }px rgba( ${ boxShadowColor }, ${ customBoxShadow.opacity / 100 } )` : null
			};
			...
			return (
				<div className={ containerClasses } style={ { '--ttb-box-shadow-color': boxShadowColor } }>
					<div className={ rowClasses } style={ rowStyle }>
						...
						<blockquote className={ contentColumnClasses } style={ contentColumnStyle }>
							{
								style === 'full' &&
